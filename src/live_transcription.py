...
def answerquestion(conn, context):
    while True:
        while True:
            question = conn.recv()
            if question:
...
if __name__ == "__main__":
    ...
    # whisp = multiprocessing.Process(
    #     target=transcribe, args=(to_mic, to_question, model)
    # )
    # QA = multiprocessing.Process(target=answerquestion, args=(from_whisper, context))
